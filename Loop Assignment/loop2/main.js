...
var isCorrect = false; //rndNum===maxValue? false : true;
//-------------------------------------------predefined guesses-----------------------
var storedGuesses = [3, 5, 7, 9];
//--------------------------------------------Iterate over guesses---------------------
var x = 0;
while (x < storedGuesses.length && !isCorrect) {
    //-----------------------------------------------Check the user's guess-----------------
    if (storedGuesses[x] === rndNum) {
        console.log("Well Done! You guess ".concat(storedGuesses[x], " is correct."));
    }
    else if (storedGuesses[x] > rndNum) {
        console.log("".concat(storedGuesses[x], ", your guessed number is high, try again."));
    }
    else {
        console.log("".concat(storedGuesses[x], " is too low, try again."));
    }
    x++;
}
;
